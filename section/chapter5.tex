...
\item Jelaskan dengan kata dan praktek cara menambahan data training dari file yang dimasukkan kepada variabel dalam rangka melatih model Doc2Vec.
	...
	\subitem Dimana Pada source code diatas sama saja akan melakukan pemanggilan data training akan tetapi dalam folder yang berbeda, sehingga dapat menambah data training tersebut. Untuk output dari source code tersebut dapat dilihat pada figure \ref{YNC5-23}
	...
	\subitem Dimana pada source code diatas juga akan memanggil data training pada folder stanfordSentimentTreebank, sehingga akan menambah data pada data trainingnya. Untuk output dari source code tersebut dapat dilihat pada figure \ref{YNC5-24}
...
\item Jelaskan dengan kata dan praktek mengapa harus dilakukan pengocokan dan pembersihan data.
	...
	\subitem Dimana pada source code tersebut akan dilakukannya pengocokan data agar kita dapat mendapatkan data yang random sehingga membuat hasil presentasenya menjadi lebih baik. Output dari source code diatas dapat dilihat pada figure \ref{YNC5-25}.
...
Tujuan pembersihan data adalah untuk melakukan pengosongan file agar meringankan ram pada laptop kita, dikarenakan laptop kita telah melakukan running data sebanyak 3000000 data, agar tidak memperlambat kinerja pada langkag berikutnya maka dari itu dilakukanlah permbersihat data. Output dari source code tersebut dapat dilihat pada figure \ref{YNC5-26}.
...
\item Jelaskan dengan kata dan praktek kenapa model harus di save dan kenapa temporari training harus dihapus.
	...
	\subitem Dalam souce code diatas berfungsi untuk melakukan penyimpanan pada data training yang sudah kita latih, supaya nantinya kita bisa melakukan load saat ingin membukanya dan tidak perlu melakukan pelatihan dari awal lagi. Data yang kita simpan akan terlihat seperti pada figure \ref{YNC5=27}
	...
	\subitem Untuk menghapus temporari  training perhatikan source code dibawah,

	\begin{verbatim}
		model.delete_temporary_training_data(keep_inference=True)
	\end{verbatim}		

Tujuan dalam menghapus temporary training data yaitu untuk meringan ram kita dengan cara menghapus data training yang sudah kita latih sebelumnya. Melakuakan training data kembali dapat membuat kinerja pada laptop kita menurun maka dari itu data yang sudah tidak terpakai sebaiknya dihapus guna mempercepat kinerja pada latop kita. Untuk ouput dari source code tersebut dapat dilihat pada figure \ref{YNC5-26}
...
\item Jelaskan dengan kata dan praktek maksud dari infer\_code.
	...
	\subitem Dimana infer\_code disini berfungsi untuk melakukan pebandingan kata dengan vector menggunakan model infer\_vector. Data vector yang akan dibandingkan adalah data training yang sudah kita simpan. Infer\_code yang dipadukan dengan infer\_vector dapat melakukan kalkulasi vector yang telah kita cantumkan pada model yang sudah kita buat. Untuk output dari source code tersebut dapat dilihat pada figure \ref{YNC5-28}
