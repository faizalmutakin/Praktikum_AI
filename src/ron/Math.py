# In[1]:import lib
import csv #Untuk melakukan import library csv
from PIL import Image as pil_image #Untuk import metode image dari PIL dengan inisialisasi pil_image
import keras.preprocessing.image #Untuk import library keras dengan metode preprocessing image

# In[2]:load all images (as numpy arrays) and save their classes

imgs = [] #variabel imgs
classes = [] #variabel classes
with open('HASYv2/hasy-data-labels.csv') as csvfile: #Untuk memanggil file hasy tersebut dengan ekstensi .csv
    csvreader = csv.reader(csvfile) #Untuk membaca file tersebut
    i = 0
...
import random #Untuk import library random
random.shuffle(imgs) #Untuk melakukan pengacakan atau shuffle terhadap data tersebut
split_idx = int(0.8*len(imgs)) 
train = imgs[:split_idx] #Data training
test = imgs[split_idx:] #Data testing

# In[4]: 

import numpy as np #Untuk melakukan import library numpy dengan inisialisasi np
#Pembagian data training inputan
train_input = np.asarray(list(map(lambda row: row[2], train))) 
test_input = np.asarray(list(map(lambda row: row[2], test)))
#Pembagian data training sebagai output
train_output = np.asarray(list(map(lambda row: row[1], train)))
test_output = np.asarray(list(map(lambda row: row[1], test)))

# In[5]: import encoder and one hot
from sklearn.preprocessing import LabelEncoder #Import library sklearn dengan metode LabelEncoder
from sklearn.preprocessing import OneHotEncoder #Import library sklearn dengan metode OneHotEncoder
