...
def extract_features_song(f):
    #melakukan load data pada variabel f
    y, _ = librosa.load(f)

    # membuat feature dari y yang dimana y itu data dari f
    mfcc = librosa.feature.mfcc(y)
    # memasukkan data kembali kedalam variabel mfcc dalam bentuk np
    mfcc /= np.amax(np.absolute(mfcc))
    # akan melakukan returen dan datanya berbentuk ndarray dari fariabel mfcc
    # serta membatasi data pertamanya sebanyakl 25.000
...
# In[3]
filename = 'D:/Perkuliahan/Semester 6/Kecerdasan Buatan/PC6/genres/blues/blues.00000.au'
x_blues, sr_blues = librosa.load(filename, duration=30)
...
# Cara penanganannya adalah install modul keras dengan perintah seperti berikut
